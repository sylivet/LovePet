<?php
      ...
      //建立SQL
      if($FK_HELTH_CHECK_ID&&$FK_HELTH_CHECK_ID2){
      ...
      $statement->execute();
     }else if($FK_HELTH_CHECK_ID&&!$FK_HELTH_CHECK_ID2){
      $sql = "INSERT INTO HOSPITAL_ORDER(FK_MEMBER_ID,FK_PET_ID, CREATE_DATE, BOOKING_DATE, FK_SET_MENU_ID, FK_HELTH_CHECK_ID, TOTAL_PRICE, ORDER_STATUS) VALUES (?, ?, ?, ?, ?, ?, ?, 0)";
      //執行
      $statement = $pdo->prepare($sql);  
      $statement->bindValue(1 , $FK_MEMBER_ID); 
      $statement->bindValue(2 , $FK_PET_ID); 
      ...
      $statement->bindValue(5 , $FK_SET_MENU_ID); 
      $statement->bindValue(6 , $FK_HELTH_CHECK_ID); 
      $statement->bindValue(7 , $TOTAL_PRICE); 
      $statement->execute();     
      }else{
            $sql = "INSERT INTO HOSPITAL_ORDER(FK_MEMBER_ID,FK_PET_ID, CREATE_DATE, BOOKING_DATE, FK_SET_MENU_ID, TOTAL_PRICE, ORDER_STATUS) VALUES (?, ?, ?, ?, ?, ?, 0)";      
            //執行
            $statement = $pdo->prepare($sql);  
            ...
            $statement->bindValue(4 , $BOOKING_DATE); 
            $statement->bindValue(5 , $FK_SET_MENU_ID); 
            $statement->bindValue(6 , $TOTAL_PRICE); 
